...
$$ (1-e^x)\sin(y)dy=e^x\cos^3(y)dx $$

Тепер ми можемо розділити змінні:

$$ \frac{\sin(y)}{\cos^3(y)}dy=\frac{e^x}{1-e^x}dx $$

Інтегруючи обидві частини, отримаємо:

$$ \int\frac{\sin(y)}{\cos^3(y)}dy=\int\frac{e^x}{1-e^x}dx $$
