...
void prnhead(void)
{
  ...
  outfile << endl;
  outfile << "<HEAD>" << endl;
  outfile << "<TITLE>Результати моделювання</TITLE>" << endl;
  outfile << "</HEAD>" << endl;
  outfile << endl;
  outfile << "<BODY>" << endl;
  outfile << endl;
  ...
  outfile << "<!-- Виведення інформації про розробників -->" << endl;
  outfile << "<TABLE align=right width=100% border=3 cellspacing=0 cellpadding=0>" << endl;
  outfile << "<THEAD>" << endl;
  outfile << "<TR align=center>" << endl;
  outfile << "<TH width=70% align=left><B>СІМ-СІ++ v1.2</B></TH>" << endl;
  outfile << "<TH width=15%><EM>НУ\"ЗП\"</EM></TH>" << endl;
  outfile << "<TH width=15%><EM>2024</EM></TH>" << endl;
  outfile << "</TR>" << endl;
  outfile << "</THEAD>" << endl;
  outfile << "<TR>" << endl;
  outfile << "<TD colspan=3><NOBR>Ж. К. Камінська</NOBR><BR>" << endl;
  outfile << "<NOBR>С. М. Сердюк</NOBR></TD>" << endl;
  outfile << "</TR>" << endl;
  outfile << "</TABLE>" << endl;
  outfile << endl;
  outfile << "</TD>" << endl;
...
{
  {
    ...
    outfile << "<THEAD>" << endl;
    outfile << "<TR align=center>" << endl;
    outfile << "<TD> Поточна довжина </TD> <TD> Максимальна довжина </TD>" << endl;
      ...
      {
        {
          ...
          outfile << "<TR>" << endl;
          outfile << "<TD colspan=8> Параметри: <BR>" << endl;
          ...
          outfile << "&nbsp;PQ: ";
          if (with1.pq[1] != nil)
            outfile << with1.pq[1]->name;
          else
            outfile << "NIL";
          if (with1.pq[2] != nil)
            outfile << ' ' << with1.pq[2]->name << "<BR>" << NL;
          else
            outfile << " NIL"
                    << "<BR>" << NL;
          outfile << "&nbsp;PF: ";
          if (with1.pf[1] != nil)
            outfile << with1.pf[1]->name;
          else
            outfile << "NIL";
          if (with1.pf[2] != nil)
            outfile << ' ' << with1.pf[2]->name << "<BR>" << NL;
          else
            outfile << " NIL"
                    << "<BR>" << NL;
          outfile << "&nbsp;PS: ";
          if (with1.ps[1] != nil)
            outfile << with1.ps[1]->name;
          else
            outfile << "NIL";
          if (with1.ps[2] != nil)
            outfile << ' ' << with1.ps[2]->name << "<BR>" << NL;
          else
            outfile << " NIL"
                    << "<BR>" << NL;
          outfile << "</TD> </TR>" << endl;
...
static void printgraf(phistogram &hist)
{
  outfile << "<BR>" << endl;
  printtobmp(hist);
  outfile << "<IMG SRC=\"" << bmp_filename << "\" align=center width=100% alt=\"Гістограма\">" << endl;
  /*    array_newl workline,
      board,
      midle,
      limiter;
...
void prnhist(phistogram hist)
{
  ...
  len = 15;
  outfile << "<H2 align=center> Гістограма " << hist->name << " </H2>" << endl;
  ...
  //     outfile << "     ____________________________________" << NL;
  {
    ...
    outfile << "<TR> <TD width=50%> Діапазон вимірювань </TD> <TD width=50%> " << SHOW(3, with.minx) << " .. " << SHOW(3, with.maxx) << "</TD> </TR>" << NL;
    outfile << "<TR> <TD> Загальна кількість входжень </TD> <TD> " << SHOW0(with.total) << "</TD> </TR>" << NL;
    if (with.total > 1)
    {
      outfile << "<TR> <TD> Середнє значення </TD> <TD> " << SHOW(5, with.sum / with.total) << "</TD> </TR> " << endl;
      outfile << "<TR> <TD> Дисперсія </TD> <TD> " << SHOW(5, (with.sumsqr - with.sum * with.sum / with.total) / (with.total - 1)) << "</TD> </TR> " << endl;
      outfile << "<TR> <TD> Число виходів за інтервал зліва </TD> <TD> " << SHOW0(with.x[0]) << " </TD> </TR> " << endl;
      outfile << "<TR> <TD> Число виходів за інтервал справа </TD> <TD> " << SHOW0(with.x[with.ihint]) << " </TD> </TR> " << endl;
      outfile << "</TABLE>" << endl;
      ...
      outfile << "<CAPTION> Таблиця спостережень </CAPTION>" << endl;
      k = 1;
      j = 1;
      y = with.minx;
...
{
  {
    ...
    if (with.ci != 0)
    {
      ...
      case facility::free:
        outfile << "FREE";
        break;
      case facility::seized:
        outfile << "SEIZED";
        break;
      case facility::preempted:
        outfile << "PREEMPTED";
        break;
      }
    }
...
void prwaitl()
{
  ...
  for (i = 1; i <= evemax; i++)
    if (waitl[i] != nil)
    {
      outfile << "<H2> Системний список " << formatstr(" WAITL", 14) << formatint(i, 5) << "</H2>" << NL;
      prnlt(waitl[i]);
    }
}
...
void prnuserlt()
{
  ...
  for (i = 1; i <= userlist.ll; i++)
  {
    ...
    outfile << "<H2> Користувацький список " << userlist.first->p.name << " </H2>" << NL;
...
void lfprint(plistf lf)
{
...

  {
    ...
    if (with.first != nil)
      for (i = 1; i <= with.p.ll; i++)
      {
        outfile << "<H2 align=center>Список приладу " << with.first->name << "</H2>" << NL;
        prnlt(with.first->fl);
        if (with.first->inter != nil)
        {
          outfile << "<H2 align=center>Список перерваних транзактів " << with.first->name << "</H2>" << NL;
...
void lsprint(plists ls)
{
...

  {
    ...
    if (with.first != nil)
      for (i = 1; i <= with.p.ll; i++)
      {
        outfile << "<H2 align=center>Список накопичувача " << with.first->name << "<BR>" << NL;
...
void prnlq(plistq lq)
{
  ...
  outfile << "<!-- Виведення інформації про черги-->" << endl;
  outfile << "<BR>" << endl;
  outfile << "<TABLE border=3 width=100% cellspacing=0 cellpadding=0>" << endl;
  outfile << "<CAPTION>Черги</CAPTION>" << endl;
  outfile << "<THEAD>" << endl;
  outfile << "<TR align=center>" << endl;
  outfile << "<TH align=left width=20% rowspan=2>&nbsp;Черга</TH>" << endl;
  outfile << "<TD width=16%>Кількість входжень</TD>" << endl;
  outfile << "<TD width=16%>Макс. довжина</TD>" << endl;
  outfile << "<TD width=16%>Середній час очікування</TD>" << endl;
  outfile << "<TD width=16% rowspan=2>Середня довжина</TD>" << endl;
  outfile << "<TD width=16% rowspan=2>% входжень у порожню чергу</TD>" << endl;
  outfile << "</TR>" << endl;
  outfile << "<TR align=center>" << endl;
  outfile << "<TD>З нульовим часом очікування</TD>" << endl;
  outfile << "<TD>Поточна довжина</TD>" << endl;
  outfile << "<TD>Без урахування нульових входжень</TD>" << endl;
  outfile << "</TR>" << endl;
  outfile << "</THEAD>" << endl;
...
void prnls(plists ls)
{
  ...
  outfile << "<!-- Виведення інформації про накопичувачі-->" << endl;
  outfile << "<BR>" << endl;
  outfile << "<TABLE width=100% border=3 cellspacing=0 cellpadding=0>" << endl;
  outfile << "<CAPTION>Накопичувачі</CAPTION>" << endl;
  outfile << "<THEAD align=center>" << endl;
  outfile << "<TR>" << endl;
  outfile << "<TH rowspan=2 align=left width=20%>&nbsp;Накопичувач</TH>" << endl;
  outfile << "<TD rowspan=2 width=8%>Ємність</TD>" << endl;
  outfile << "<TD rowspan=2 width=8%>Завантаженість</TD>" << endl;
  outfile << "<TD rowspan=2 width=16%>Середній час перебування</TD>" << endl;
  outfile << "<TD colspan=3>Вміст</TD>" << endl;
  outfile << "<TD rowspan=2 width=16%>Кількість входжень</TD>" << endl;
  outfile << "</TR>" << endl;
  outfile << "<TR>" << endl;
  outfile << "<TD>Поточн.</TD>" << endl;
  outfile << "<TD>Макс.</TD>" << endl;
  outfile << "<TD>Середн.</TD>" << endl;
  outfile << "</TR>" << endl;
  outfile << "</THEAD>" << endl;
...
void prnlf(plistf lf)
{
  ...
  outfile << "<!-- Виведення інформації про прилади -->" << endl;
  outfile << "<BR>" << endl;
  outfile << "<TABLE width=100% border=3 cellspacing=0 cellpadding=0>" << endl;
  outfile << "<CAPTION>Прилади</CAPTION>" << endl;
  outfile << "<THEAD align=center>" << endl;
  outfile << "<TR>" << endl;
  outfile << "<TH align=left width=20%>&nbsp;Прилад</TH>" << endl;
  outfile << "<TD width=16%>Кількість входжень</TD>" << endl;
  outfile << "<TD width=16%>Середній час обробки</TD>" << endl;
  outfile << "<TD width=16%>Завантаження</TD>" << endl;
  outfile << "<TD width=16%>Кількість захоплень</TD>" << endl;
  outfile << "<TD width=16%>Стан</TD>" << endl;
  outfile << "</TR>" << endl;
  outfile << "</THEAD>" << endl;
...
void prnq1(pqueue que)
{
  ...
  outfile << "<TH align=left width=20% rowspan=2>&nbsp;Черга</TH>" << endl;
  outfile << "<TD width=16%>Кількість входжень</TD>" << endl;
  outfile << "<TD width=16%>Максимальна довжина</TD>" << endl;
  outfile << "<TD width=16%>Середній час очікування</TD>" << endl;
  outfile << "<TD width=16% rowspan=2>Середня довжина</TD>" << endl;
  outfile << "<TD width=16% rowspan=2>% входжень у порожню чергу</TD>" << endl;
  outfile << "</TR>" << endl;
...
void prnf1(pfacility fac)
{
  ...
  outfile << "<TD width=16%>Середній час обробки</TD>" << endl;
  outfile << "<TD width=16%>Завантаження</TD>" << endl;
  outfile << "<TD width=16%>Число захоплень</TD>" << endl;
  outfile << "<TD width=16%>Стан</TD>" << endl;
  outfile << "</TR>" << endl;
  outfile << "</THEAD>" << endl;
  prnf(fac);
...
void prns1(pstorage st)
{
  ...
  outfile << "<TH rowspan=2 align=left width=20%>&nbsp;Накопичувач</TH>" << endl;
  outfile << "<TD rowspan=2 width=8%>Ємність</TD>" << endl;
  outfile << "<TD rowspan=2 width=8%>Завантаження</TD>" << endl;
  outfile << "<TD rowspan=2 width=16%>Середній час перебування</TD>" << endl;
  outfile << "<TD colspan=3>Вміст</TD>" << endl;
  outfile << "<TD rowspan=2 width=16%>Кількість входжень</TD>" << endl;
  outfile << "</TR>" << endl;
...
void printall()
{
  ...
  outfile << "<CAPTION align=center>&nbsp;Загальні параметри середовища:&nbsp;</CAPTION>" << endl;
  outfile << "<TR width=50%> <TD>Поточний час</TD> <TD width=50%>" << SHOW(3, systime) << "</TD> </TR>" << endl;
  outfile << "<TR> <TD>Поточна подія</TD> <TD>" << SHOW0(sysevent) << "</TD> </TR>" << endl;
  if (trans == nil)
    outfile << "<TR> <TD>Поточний транзакт</TD> <TD>NIL</TD> </TR>" << endl;
  else
    outfile << "<TR> <TD>Поточний транзакт</TD> <TD>" << SHOW0(trans->nom) << "</TD> </TR>" << endl;
  outfile << "<TR> <TD>Усього подій</TD> <TD>" << SHOW0(eventall) << "</TD> </TR>" << endl;
  outfile << "<TR> <TD>Час моделювання</TD> <TD>" << SHOW(2, realtime) << " сек.</TD> </TR>" << endl;
  outfile << "<TR> " << endl;
  outfile << "<TD>Середній час виконання події</TD>" << endl;
  if (realtime == 0.0)
    outfile << "<TD>" << SHOW(5, realtime) << " сек/подія</TD>" << endl;
  else
    outfile << "<TD>" << SHOW(5, realtime / eventall) << " сек/подія</TD>" << endl;
  outfile << "</TR>" << endl;
  outfile << "</TABLE>" << endl;
  ...
  outfile << "<!-- Виведення кількості подій -->" << endl;
  for (int i = 0; i < (((ievemax % 10) == 0) ? (ievemax / 10) : (ievemax / 10 + 1)); i++)
  {
    ...
    outfile << "<TABLE border=3 width=100% cellspacing=0 cellpadding=0>" << endl;
    outfile << "<TR align=center>" << endl;
    outfile << "<TH width=20% align=left>&nbsp;ПОДІЯ</TH>" << endl;
    for (int j = 1; j <= 10; j++)
      outfile << "<TD width=8%>" << SHOW0(segm + j) << "</TD>" << endl;
    outfile << "</TR>" << endl;
    outfile << "<TR align=center>" << endl;
    outfile << "<TH align=left>&nbsp;УСЬОГО</TH>" << endl;
    for (int j = 1; j <= 10; j++)
      if (segm + j <= ievemax)
  ...
  if (errtest || trace)
  {
    //        outfile << stars1 << NL;
    //        outfile << stars2 << NL;
    outfile << "<H1 align=center>Системні списки</H1>" << NL;
    //        outfile << stars2 << NL;
    //        outfile << stars1 << NL;
    //        outfile << NL;
...
void starttrace(void)
{
  if (trace)
    return;
  outfile << "<!-- Виведення трасування -->" << endl;
  outfile << "<BR>" << endl;
  outfile << "<TABLE border=3 width=100% cellspacing=0  cellpadding=0>" << endl;
  outfile << "<CAPTION>Трасування</CAPTION>" << endl;
  outfile << "<THEAD>" << endl;
  outfile << "<TR>" << endl;
  outfile << "<TD width=10%>&nbsp;№ події&nbsp;</TD>" << endl;
  outfile << "<TD width=10%>&nbsp;Системний час&nbsp;</TD>" << endl;
  outfile << "<TD width=70%>&nbsp;Опис&nbsp;</TD>" << endl;
  outfile << "</TR>" << endl;
  outfile << "</THEAD>" << endl;
  trace = true;
}
...
void tracer(int tracecode)
{
  ...
  {
  case 1:
    outfile << ' ' << tr << ' ' << SHOW0(tracerec.t->nom) << " поміщений у модель (EVE=" << SHOW0(tracerec.t->eve) << "," << ne << SHOW(2, tracerec.t->nexttime) << ")." << NL;
    break;
  case 2:
    outfile << " Створений" << tr << ' ' << SHOW0(tracerec.t->nom) << " (EVE=" << SHOW0(tracerec.t->eve) << ',' << ne << SHOW(2, tracerec.t->nexttime) << ")." << NL;
    break;
  case 3:
    outfile << " Знищений" << tr << ' ' << SHOW0(trans->nom) << NL;
    break;
  case 4:
    outfile << tr << ' ' << SHOW0(trans->nom) << " затриманий на " << SHOW(3, tracerec.r) << " (" << ne << SHOW(3, trans->nexttime) << ")." << NL;
    break;
  case 5:
    outfile << tr << formatint(trans->nom, 5) << "  Eve=" << formatint(trans->eve, 4) << "   поміщений до списку Waitl[" << formatint(tracerec.e, 4) << ']' << NL;
    break;
  case 6:
    outfile << tr << formatint(trans->nom, 5) << "   спрямований на виконання події" << formatint(tracerec.e, 5) << NL;
    break;
  case 7:
    outfile << tr << formatint(trans->nom, 5) << " намагався захопити" << fa << formatstr(tracerec.f->name, 9) << ", займаний" << tr << "ом " << formatint(tracerec.f->transpoint->nom, 5) << NL;
    break;
  case 8:
    outfile << fa << formatstr(tracerec.f->name, 9) << " захоплений" << tr << "ом" << formatint(trans->nom, 5) << " обробка" << tr << 'а' << formatint(tracerec.f->transpoint->nom, 5) << " перервана" << NL;
    break;
  case 9:
    outfile << tr << formatint(trans->nom, 5) << "  зайняв " << fa << formatstr(tracerec.f->name, 9) << NL;
    break;
  case 10:
    outfile << tr << formatint(trans->nom, 5) << "  звільнив " << fa << formatstr(tracerec.f->name, 9) << NL;
    break;
  case 11:
    outfile << tr << formatint(tracerec.f->transpoint->nom, 5) << " повернутий у" << fa << formatstr(tracerec.f->name, 9) << " на доопрацювання" << NL;
    break;
  case 12:
    outfile << tr << formatint(trans->nom, 5) << " залишив чергу " << formatstr(tracerec.q->name, 9) << NL;
    break;
  case 13:
    outfile << tr << formatint(trans->nom, 5) << " став у чергу " << formatstr(tracerec.q->name, 9) << NL;
    break;
  case 14:
    outfile << tr << formatint(trans->nom, 5) << " намагався зайняти" << fa << formatstr(tracerec.f->name, 9) << NL;
    break;
  case 15:
    outfile << tr << formatint(trans->nom, 5) << "  зайняв" << fa << formatstr(tracerec.f->name, 9) << NL;
    break;
  case 16:
    outfile << tr << formatint(trans->nom, 6) << "  зайняв" << formatint(tracerec.n, 4) << " комірок у накопичувачі " << formatstr(tracerec.s->name, 9) << NL;
    break;
  case 17:
    outfile << tr << formatint(trans->nom, 6) << " намагався увійти в накопичувач " << formatstr(tracerec.s->name, 9) << NL;
    break;
  case 18:
    outfile << tr << formatint(trans->nom, 6) << " звільнив" << formatstr(tracerec.n, 5) << " комірок накопичувача " << formatstr(tracerec.s->name, 9) << NL;
    break;
  case 19:
    outfile << tr << formatint(tracerec.t->nom, 6) << " переведений зі списку накопичувача в Current" << NL;
    break;
  case 20:
    outfile << " Параметру ";
    break;
  ...
  case 206:
    ...
    break;
  case 207:
    outfile << formatint(tracerec.n, 2) << "] усіх членів ансамблю " << formatint(trans->ans, 5) << " присвоєно значення ";
    break;
  case 208:
    outfile << trans->pi[tracerec.n];
    break;
  case 209:
    outfile << formatstr(btos(trans->pb[tracerec.n]), 7);
    break;
  case 210:
    outfile << trans->pr[tracerec.n];
    break;
  case 211:
    outfile << '^' << formatstr(trans->pf[tracerec.n]->name, 9);
    break;
  case 212:
    outfile << '^' << formatstr(trans->pq[tracerec.n]->name, 9);
    break;
  case 213:
    outfile << '^' << formatstr(trans->ps[tracerec.n]->name, 9);
    break;
  case 214:
  {
    outfile << NL;
  ...
  case 21:
  {
    transact &with1 = *tracerec.t;
    outfile << formatint(with1.nom, 6) << formatint(with1.prty, 6) << formatint(with1.eve, 7) << formatfloat(with1.nexttime, 12, 3) << formatint(with1.ans, 6) << formatint(with1.nans, 7) << formatstr(btos(with1.testprty), 10) << formatint(with1.translist->p.name, 16) << NL;
  }
  break;
  case 22:
    outfile << " Для" << tr << 'у' << formatint(trans->nom, 5) << ", ансамбль" << formatint(trans->ans, 5) << " створено" << formatint(tracerec.n, 4) << " нащадків,"
            << " які надіслані за адресою" << formatint(tracerec.e, 6) << NL;
    break;
  case 23:
    outfile << " Розпочато збір членів ансамблю" << formatint(trans->ans, 5) << " у події" << formatint(sysevent, 6) << NL;
    break;
  case 24:
    outfile << tr << formatint(trans->nom, 5) << ", ансамбль" << formatint(trans->ans, 5) << " виведений з моделі у події" << formatint(sysevent, 6) << NL;
    break;
  case 25:
    outfile << " Закінчено збір членів ансамблю" << formatint(trans->ans, 5) << " у події" << formatint(sysevent, 6) << NL;
    break;
  case 26:
  {
    outfile << " Усім членам ансамблю" << formatint(trans->ans, 5) << " встановлено пріоритет" << formatint(trans->prty, 4) << NL;
  ...
  {
    transact &with1 = *tracerec.t;
    outfile << formatint(with1.nom, 6) << formatint(with1.prty, 6) << formatint(with1.eve, 7) << formatfloat(with1.nexttime, 12, 3) << formatint(with1.ans, 6) << formatint(with1.nans, 7) << formatstr(btos(with1.testprty), 10) << formatstr(tracerec.t->translist->p.name, 16) << NL;
  }
  break;
  case 28:
    outfile << tr << formatint(trans->nom, 5) << " поміщений до списку SignList" << formatint(tracerec.n, 4) << NL;
    break;
  case 29:
    outfile << tr << "и, які очікували сигналу" << formatint(tracerec.n, 4) << ", переведені до списку Current" << NL;
    break;
  case 30:
    outfile << " Немає " << tr << "ів, які очікують сигналу" << formatint(tracerec.n, 4) << NL;
    break;
  }
  outfile << "</TD>" << endl;
  outfile << "</TR>" << endl;
...
static void errglob()
{
  /*    outfile << formatstr("    ",15) << "        *****************************" << NL;
      ...
      outfile << formatstr("    ",15) << "        *                           *" << NL;
      outfile << formatstr("    ",15) << "        *****************************" << NL; */
  outfile << "<H1> Діагностика помилок </H1>" << endl;
}
...
static void errmsg(int i, bool b, bool &ok, int &errcode)
{
  ...
  // ok=f;
  if (!f)
    outfile << "<P> Error code #" << formatint(errcode, 5) << "</P>" << NL;
  else
  {
    for (j = 1; j <= i; j++)
      f >> line /* >> NL */;
...
void error1(int &errcode)
{
  ...
  {
  case 101:
    errmsg(1, true, ok, errcode);
    break;
  case 102:
    errmsg(2, true, ok, errcode);
    break;
  case 103:
    errmsg(3, true, ok, errcode);
    break;
  case 201:
    errmsg(4, false, ok, errcode);
    break;
  case 202:
    errmsg(5, false, ok, errcode);
    break;
  case 203:
    errmsg(6, false, ok, errcode);
    break;
  case 204:
    errmsg(7, false, ok, errcode);
    break;
  case 205:
    errmsg(8, false, ok, errcode);
    break;
  case 206:
    errmsg(9, false, ok, errcode);
    break;
  case 207:
    errmsg(10, false, ok, errcode);
    break;
  case 208:
    errmsg(11, false, ok, errcode);
    break;
  case 209:
    errmsg(12, false, ok, errcode);
    break;
  case 301:
    errmsg(13, false, ok, errcode);
    break;
  case 302:
    errmsg(14, false, ok, errcode);
    break;
  case 401:
    errmsg(15, false, ok, errcode);
    break;
  case 402:
    errmsg(16, true, ok, errcode);
    break;
  case 403:
    errmsg(17, true, ok, errcode);
    break;
  case 404:
    errmsg(18, true, ok, errcode);
    break;
  case 405:
    errmsg(19, true, ok, errcode);
    break;
  case 501:
    errmsg(20, true, ok, errcode);
    break;
  case 502:
    errmsg(21, true, ok, errcode);
    break;
  case 601:
    errmsg(22, true, ok, errcode);
    break;
  case 602:
    errmsg(23, true, ok, errcode);
    break;
  case 701:
    errmsg(24, true, ok, errcode);
    break;
  case 702:
    errmsg(25, true, ok, errcode);
    break;
  case 711:
    errmsg(26, true, ok, errcode);
    break;
  case 712:
    errmsg(27, true, ok, errcode);
    break;
  case 713:
    errmsg(28, true, ok, errcode);
    break;
  case 714:
    errmsg(29, true, ok, errcode);
    break;
  case 715:
    errmsg(30, false, ok, errcode);
    break;
  case 716:
    errmsg(31, true, ok, errcode);
    break;
  case 717:
    errmsg(32, true, ok, errcode);
    break;
  case 801:
    errmsg(33, true, ok, errcode);
    break;
  case 802:
    errmsg(34, true, ok, errcode);
    break;
  case 7301:
    errmsg(35, true, ok, errcode);
    break;
  case 7302:
    errmsg(36, true, ok, errcode);
    break;
  }
}
...
void error(int errcode)
{
  outfile << "<P>У процесі моделювання сталася помилка #" << errcode << " </P>" << endl;
  outfile << "<A href=\"error.html\"> Діагностика помилок </A>" << endl;
  outfile.close();
...
void infuture(ptransact &t)
{
  ...
  nt = t->nexttime;
  {
    ...
    if (with.first == nil)
      inlt(future, t);
    else if ((with.first->nexttime > nt) || ((with.first->nexttime == nt) && (pt > with.first->prty)))
      inlt(future, t);
    else
    {
      t1 = with.first;
      while ((nt < with.first->pred->nexttime) || ((nt == with.first->pred->nexttime) && (pt > with.first->pred->prty)))
...
void incurrent(ptransact &t)
{
...

  {
    ...
    if (with.first == nil)
      inlt(current, t);
    else if (t->prty > with.first->prty)
    {
      inlt(current, t);
      if (trans != nil)
      {
...
void outfac(pfacility &f)
{
  ...
  if (f->test)
  {
    ...
    if (with.ci != 0)
      with.mtime = with.timef / with.ci;
    else
      with.mtime = 0.0;
    if (systime != resettime)
      with.pro = with.timef / (systime - resettime);
    else
      with.pro = 0.0;
  }
}
...
void outqueue(pqueue &q)
{
  if (q->test)
  {
    ...
    else
    {
      ...
      if (systime != resettime)
        with.lm = with.timeq / (systime - resettime);
      else
        with.lm = with.lq;
      if (with.ci != 0)
        with.mtime = with.timeq / with.ci;
      else
...
void seize(pfacility &f)
{
  ...
  if (f->test)
  {
    ...
    if (with.status != with.free)
    {
      ...
      if (with.fl->first == nil)
        inlt(with.fl, trans);
      else if (with.fl->first->prty < trans->prty)
        inlt(with.fl, trans);
      else
      {
        t = with.fl->first;
  ...
  if (st->test)
  {
    ...
    else
    {
      ...
      if (with.slt->first == nil)
        inlt(with.slt, trans);
      else if (with.slt->first->prty < trans->prty)
        inlt(with.slt, trans);
      else
      {
        t = with.slt->first;
    ...
    }
    {
      ...
      if (with.testprty)
      {
        while (((with.prty > with.pred->prty) && (with.translist != future) &&
                (t != with.translist->first)) ||
               ((with.translist == future) &&
        ...
        while (((with.prty < with.sled->prty) && (with.translist != future) &&
                (with.sled != with.translist->first)) ||
               ((with.translist == future) &&
...
void tabulate(phistogram hist, real t)
{
...

  {
    ...
    if (t < with.minx)
      ...
      with.x[with.ihint] = with.x[with.ihint] + 1;
    else
    {
...
void initlist(int itr)
{
  ...
  {
    dialogus = false;
    output << " Enter a name for the Report HTML file: ";
    input >> file_name /* >> NL */;
    const char *ext = ".html";
