...
void printall()
{
  ...
  if (trans == nil)
    ...
    outfile << "<TR> <TD>Текущий транзакт</TD> <TD>" << SHOW0(trans->nom) << "</TD> </TR>" << endl;
  ...
  if (realtime == 0.0)
    ...
    outfile << "<TD>" << SHOW(5, realtime / eventall) << " сек/событие</TD>" << endl;
  outfile << "</TR>" << endl;
  outfile << "</TABLE>" << endl;
...
void tracer(int tracecode)
{
  ...
  {
  case 1:
    outfile << ' ' << tr << ' ' << SHOW0(tracerec.t->nom) << " помещен в модель (EVE=" << SHOW0(tracerec.t->eve) << "," << ne << SHOW(2, tracerec.t->nexttime) << ")." << NL;
    break;
  case 2:
    outfile << " Создан" << tr << ' ' << SHOW0(tracerec.t->nom) << " (EVE=" << SHOW0(tracerec.t->eve) << ',' << ne << SHOW(2, tracerec.t->nexttime) << ")." << NL;
    break;
  case 3:
    outfile << " Уничтожен" << tr << ' ' << SHOW0(trans->nom) << NL;
    break;
  case 4:
    outfile << tr << ' ' << SHOW0(trans->nom) << " задержен на " << SHOW(3, tracerec.r) << " (" << ne << SHOW(3, trans->nexttime) << ")." << NL;
    break;
  case 5:
    outfile << tr << formatint(trans->nom, 5) << "  Eve=" << formatint(trans->eve, 4) << "   помещен в список Waitl[" << formatint(tracerec.e, 4) << ']' << NL;
    break;
  case 6:
    outfile << tr << formatint(trans->nom, 5) << "   направлен на выполнение события" << formatint(tracerec.e, 5) << NL;
    break;
  case 7:
    outfile << tr << formatint(trans->nom, 5) << " пытался захватить" << fa << formatstr(tracerec.f->name, 9) << ", занимаемый" << tr << "ом " << formatint(tracerec.f->transpoint->nom, 5) << NL;
    break;
  case 8:
    outfile << fa << formatstr(tracerec.f->name, 9) << " захвачен" << tr << "ом" << formatint(trans->nom, 5) << " обработка" << tr << 'а' << formatint(tracerec.f->transpoint->nom, 5) << " прервана" << NL;
    break;
  case 9:
    outfile << tr << formatint(trans->nom, 5) << "  занял " << fa << formatstr(tracerec.f->name, 9) << NL;
    break;
  case 10:
    outfile << tr << formatint(trans->nom, 5) << "  освободил " << fa << formatstr(tracerec.f->name, 9) << NL;
    break;
  case 11:
    outfile << tr << formatint(tracerec.f->transpoint->nom, 5) << " возвращен в" << fa << formatstr(tracerec.f->name, 9) << " на дообработку" << NL;
    break;
  case 12:
    outfile << tr << formatint(trans->nom, 5) << " покинул очередь " << formatstr(tracerec.q->name, 9) << NL;
    break;
  case 13:
    outfile << tr << formatint(trans->nom, 5) << " встал в очередь " << formatstr(tracerec.q->name, 9) << NL;
    break;
  case 14:
    outfile << tr << formatint(trans->nom, 5) << " пытался занять" << fa << formatstr(tracerec.f->name, 9) << NL;
    break;
  case 15:
    outfile << tr << formatint(trans->nom, 5) << "  занял" << fa << formatstr(tracerec.f->name, 9) << NL;
    break;
  case 16:
    outfile << tr << formatint(trans->nom, 6) << "  занял" << formatint(tracerec.n, 4) << " ячеек в накопителе " << formatstr(tracerec.s->name, 9) << NL;
    break;
  case 17:
    outfile << tr << formatint(trans->nom, 6) << " пытался войти в накопитель " << formatstr(tracerec.s->name, 9) << NL;
    break;
  case 18:
    outfile << tr << formatint(trans->nom, 6) << " освободил" << formatstr(tracerec.n, 5) << " ячеек накопителя " << formatstr(tracerec.s->name, 9) << NL;
    break;
  case 19:
    outfile << tr << formatint(tracerec.t->nom, 6) << " переведен из списка накопителя в Current" << NL;
    break;
  case 20:
    outfile << " Параметру ";
    break;
  ...
  case 206:
    ...
    break;
  case 207:
    outfile << formatint(tracerec.n, 2) << "] всех членов ансамбля " << formatint(trans->ans, 5) << " присвоено значение ";
    break;
  case 208:
  ...
  break;
  case 22:
    outfile << " Для" << tr << 'а' << formatint(trans->nom, 5) << ", ансамбль" << formatint(trans->ans, 5) << " создано" << formatint(tracerec.n, 4) << " потомков,"
            << " которые направлены по адресу" << formatint(tracerec.e, 6) << NL;
    break;
  case 23:
    outfile << " Начат сбор членов ансамбля" << formatint(trans->ans, 5) << " в событии" << formatint(sysevent, 6) << NL;
    break;
  case 24:
    outfile << tr << formatint(trans->nom, 5) << ", ансамбль" << formatint(trans->ans, 5) << " выведен из модели в событии" << formatint(sysevent, 6) << NL;
    break;
  case 25:
    outfile << " Закончен сбор членов ансамбля" << formatint(trans->ans, 5) << " в событии" << formatint(sysevent, 6) << NL;
    break;
  case 26:
  {
    outfile << " Всем членам ансамбля" << formatint(trans->ans, 5) << " установлен приоритет" << formatint(trans->prty, 4) << NL;
  ...
  break;
  case 28:
    outfile << tr << formatint(trans->nom, 5) << " помещен в список SignList" << formatint(tracerec.n, 4) << NL;
    break;
  case 29:
    outfile << tr << "ы, ожидавшие сигнала" << formatint(tracerec.n, 4) << ", переведены в список Current" << NL;
    break;
  case 30:
    outfile << " Нет " << tr << "ов, ожидающих сигнала" << formatint(tracerec.n, 4) << NL;
    break;
  }
  outfile << "</TD>" << endl;
  outfile << "</TR>" << endl;
